...
class Parser {
  ...
  parseStatement() {
    ...
    console.log("Current token for variable declaration:", token);
  ...
  parseVariableDeclaration() {
    const variableToken = this.tokens[this.currentTokenIndex];
    this.advanceToken(); // Avanza más allá del nombre de la variable
    console.log("Parsing variable declaration:", variableToken);
    ...
    if (
      ...
      this.advanceToken(); // Avanza más allá del '='

      const expression = this.parseExpression(); // Parsea el valor asignado a la variable
      return {
      ...
      throw new Error("Expected assignment operator after variable name");
    }
  }
  ...
  expectToken(type) {
    ...
    if (token.type !== type) {
      throw new Error(
        `Expected token of type ${type}, but found ${token.type}`
      );
    }
    this.advanceToken();
    return token;
